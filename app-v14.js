(function () {
  ...
  class DemoStore {
    ...
    seed() {
      ...
      this.data[`tournaments/${tournamentId}`] = {
        name: 'Demo Ozark Travel Stein League Event', course: 'Silo Ridge', date: new Date().toISOString().slice(0, 10),
  ...
  function authStorageKey() {
    const session = state.linkToken ? `link:${state.linkToken}` : (state.spectatorMode ? 'spectator' : 'owner');
    return `travelLeagueAuth:${state.config?.projectId || 'demo'}:${session}`;
  }
  ...
  async function redeemAccessLink(token) {
    ...
    const claimPath = `accessClaims/${state.auth.uid}`;
    const existing = await state.store.get(claimPath);
    const expectedGroup = link.group ?? null;

    if (existing) {
      const matches = existing.token === token
        && existing.tournamentId === link.tournamentId
        && existing.role === link.role
        && (existing.group ?? null) === expectedGroup;
      if (!matches) throw new Error('This browser identity is already assigned to a different private scoring link. Open this link in a new Private Browsing tab.');
    } else {
      await state.store.set(claimPath, {
        uid: state.auth.uid,
        token,
    ...
    state.access = { role: link.role, group: expectedGroup };
    state.linkToken = token;
    return link;
  }

  async function resolveAccess(tournamentId) {
    // A private link always stays in its link role, even when opened on the administrator's phone.
    if (state.linkToken && state.access.role !== 'spectator') return;
  ...
  async function init() {
    ...
    const params = new URLSearchParams(location.search);
    state.linkToken = params.get('access') || null;
    state.spectatorMode = params.has('spectator');
    if (params.has('demo')) {
      state.store = new DemoStore();
      setBanner(`Demo mode connected • Ozark Travel Stein League ${APP_VERSION}`, 'connected');
    } else {
    ...
    try {
      ...
      if (!params.has('demo')) setBanner(`Connected to Firebase • Ozark Travel Stein League ${APP_VERSION}`, 'connected');
    } catch (error) {
      showFatal(error, accessToken ? 'This private group link could not be authorized. Publish the included Firestore rules, then regenerate and resend the group links.' : 'Firebase could not load your tournaments. Check Firestore Database and Security Rules.');
    }
  }
  ...
  function bindEvents() {
    ...
    $('clearAllScores').addEventListener('click', clearAllScores);
  ...
  async function clearAllScores() {
    if (!isAdmin()) { toast('Administrator access required'); return; }
    if (!state.scores.length) { toast('There are no scores to clear'); return; }
    if (!confirm(`Clear all ${state.scores.length} entered scores? The tournament, course, players, groups, handicaps, and skin-hole selections will be kept.`)) return;
    const button = $('clearAllScores');
    button.disabled = true;
    try {
      await Promise.all(state.scores.map((score) => state.store.delete(`tournaments/${state.tournamentId}/scores/${score.id}`)));
      state.scores = [];
      renderLeaderboard();
      renderGroupScore();
      renderSkins();
      ...
      showFatal(error, 'The scores could not be cleared.');
    } finally {
      button.disabled = false;
    }
  }
  ...
  function calculateSkinContest(selectedHoles, useNet) {
    ...
    selectedHoles.forEach((hole) => {
      ...
      leaders.forEach((leader) => {
        totals.set(leader.player.id, (totals.get(leader.player.id) || 0) + 1);
      });
      if (leaders.length === 1) {
        results.push({ hole, status: 'winner', winner: leaders[0], low });
      } else {
      ...
      <div class="skins-hole-list">
        ${contest.results.map((result) => {
          ...
          if (result.status === 'tie') {
            const names = result.leaders.map((entry) => escapeHtml(entry.player.name)).join(', ');
            return `<div class="skin-hole-row winner"><b>Hole ${result.hole}</b><span>${names} — tied winners at ${result.low}${useNet ? ' net' : ' gross'}</span></div>`;
          }
  ...
  init().catch((error) => showFatal(error, 'Ozark Travel Stein League could not start.'));
}());
